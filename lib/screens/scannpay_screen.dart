...
class ScanNPayScreen extends StatefulWidget {
  const ScanNPayScreen({Key key}) : super(key: key);

  @override
  _ScanNPayScreenState createState() => _ScanNPayScreenState();
}

class _ScanNPayScreenState extends State<ScanNPayScreen> {
  ...
  Future<Null> scanQR() async {
    ...
    setState(() {
      ...
      if(barcodeScanRes == "-1"){
        ...
        print("ANKIT LIKHAR : "+barcodeScanRes);
        String show = barcodeScanRes;
        print(show.toString().split('@')[0]);
        walletAddressController.text = show.toString().split('@')[0];
      }
  ...
  void sendCoinAPI()async{
    ...
    sendCoinRequest.receiver = barcodeScanRes;
